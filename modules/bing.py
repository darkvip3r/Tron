"""Python wrapper for bing web APIs

This module allows you to access Bing's web APIs through SOAP,
...
Sample usage:
>>> import bing
>>> bing.setLicense('...') # must get your own key!
...
_url = 'http://api.bing.com/search'
_namespace = 'urn:bingSearch'
_false = SOAP.booleanType(0)
_true = SOAP.booleanType(1)
_googlefile1 = ".bingkey"
_googlefile2 = "bingkey.txt"
_licenseLocations = (
    ...
    (lambda key: _contentsOf(_getScriptDir(), _googlefile1), '%s in the bing.py directory' % _googlefile1),
    (lambda key: _contentsOf(_getScriptDir(), _googlefile2), '%s in the bing.py directory' % _googlefile2)
    )

## administrative functions
def version():
    print """Pybing %(__version__)s
%(__copyright__)s
released %(__date__)s
...
options:
  -k, --key= <license key> bing license key (see important note below)
...
IMPORTANT NOTE: all bing functions require a valid license key;
visit http://www.bing.com/apis/ to get one.  %(program)s will look in
...
def getLicense(license_key = None):
    ...
    for get, location in _licenseLocations:
        ...
        if rc: return rc
    usage()
    raise NoLicenseKey, 'get a license key at http://www.bing.com/apis/'
