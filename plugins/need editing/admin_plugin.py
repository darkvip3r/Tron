...
def handler_admin_join(type, source, parameters):
	...
	if parameters:
		...
		if bot_nick:
			reply(type, source, u'I joined to ' + groupchat+u' with nick '+bot_nick+'.')
		else:
			reply(type, source, u'I joined to ' + groupchat+u' with nick '+DEFAULT_NICK+'.')
...
def handler_admin_leave(type, source, parameters):
	...
	if popups_check(groupchat):
		if reason:
			msg(groupchat, u'Leaved by '+source[2]+u' with reason:\n'+reason)
		else:
			msg(groupchat, u'Leaved by '+source[2]+'.')
	if reason:
		leave_groupchat(groupchat, u'Leaved me by '+source[2]+u' with reason:\n'+reason)
	else:
...
def handler_admin_exit(type, source, parameters):
	...
	if reason:
		for x in gch:
			if popups_check(x):
				msg(x, u'shut downed by '+source[2]+u' with reason:\n'+reason)
	else:
		for x in gch:
			if popups_check(x):
...
register_command_handler(handler_admin_join, COMM_PREFIX+'join', ['superadmin','muc','all','*'], 40, 'Join to a conference, if there is a password write that password right after the name of conference.', COMM_PREFIX+'join <conference> [pass=1234] [botnick]', [COMM_PREFIX+'join botzone@conference.jsmart.web.id', COMM_PREFIX+'join join botzone@conference.jsmart.web.id somebot', COMM_PREFIX+'join join botzone@conference.jsmart.web.id pass=1234 somebot'])
...
register_command_handler(handler_glob_msg, COMM_PREFIX+'globmsg', ['superadmin','muc','all','*'], 100, 'Send message to all conference, where the bot exist.', COMM_PREFIX+'globmsg [message]', [COMM_PREFIX+'globmsg hi all!'])
register_command_handler(handler_glob_msg_help, COMM_PREFIX+'hglobmsg', ['superadmin','muc','all','*'], 100, 'Send message to all conference, where the bot exist. The message will contain a header with a short pre help.', COMM_PREFIX+'hglobmsg [message]', [COMM_PREFIX+'hglobmsg hi all!'])
register_command_handler(handler_popups_onoff, COMM_PREFIX+'popups', ['admin','muc','all','*'], 30, 'Off (0) and On (1) message about join/leaves, restarts/off, and also global news for certain conf. Without a parameter the bot will based on current state.', COMM_PREFIX+'popups [1|0]', [COMM_PREFIX+'popups 1',COMM_PREFIX+'popups'])
...
register_command_handler(handler_changebotstatus, COMM_PREFIX+'set_status', ['admin','muc','all','*'], 20, 'Change bot status in the current conference, if two parameters are not mentioned, bots will use the default status.', COMM_PREFIX+'set_status [online|chat|away|xa|dnd] [message]', [COMM_PREFIX+'set_status away',COMM_PREFIX+'set_status away go to work'])
register_command_handler(handler_set_nick, COMM_PREFIX+'set_nick', ['superadmin','muc','all','*'], 30, 'Changes the bot nickname in the current conference.', COMM_PREFIX+'set_nick <nick>', [COMM_PREFIX+'set_nick somebot'])
register_command_handler(handler_remote, COMM_PREFIX+'remote', ['superadmin','muc','all','*'], 40, 'Allows you to remotely execute commands and aliases in other conferences on behalf of the bot and get the result. Without parameters displays a list of conferences with the numbers, instead of the full name of the conference can use a number from the list.', COMM_PREFIX+'remote <groupchat|number from the list> <comm> <parameters>', [COMM_PREFIX+'remote botzone@conference.jsmart.web.id.aq ping guy',COMM_PREFIX+'remote 2 time guy',COMM_PREFIX+'remote'])
register_command_handler(handler_redirect, COMM_PREFIX+'redirect', ['admin','muc','all','*'], 20, 'Redirects the result of a command or an alias to the specified user in private. If the alias or command is not specified and instead the text, or any false, then sends the user a message.', COMM_PREFIX+'redirect <nick>:<command>[<params>]|<mess>', [COMM_PREFIX+'redirect guy: ping lady'])
