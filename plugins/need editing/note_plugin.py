...
def get_jid(gch, nick):
	...
	qres = querynote('dynamic/'+gch+'/users.db',sql)
...
def get_notes_id(gch,jid):
	...
	notes_id = querynote('dynamic/'+gch+'/notes.db',sql)
...
def note_add(gch,jid,note,notes_id=''):
	if not notes_id:
		...
		res = querynote('dynamic/'+gch+'/notes.db',sql)
...
def get_note_state(gch):
	if not os.path.exists('dynamic/'+gch+'/notes.db'):
