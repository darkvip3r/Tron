...
try:
	import json
	register_command_handler(handler_google_trans, COMM_PREFIX+'trans', ['info','all'], 10, 'Translate from one language to another. Via Google Translate engine. Available languages for translation:\n' + ', '.join(sorted([x.encode('utf-8')+': '+y.encode('utf-8') for x,y in trans_langs.iteritems()])), COMM_PREFIX+'trans <source_lang> <target_lang> <text>', [COMM_PREFIX+'trans en ru hello', COMM_PREFIX+'trans ru en привет'])
except ImportError:
	try:
