...
def handler_changestatus(type, source, parameters):
   ...
   if check_file(file='chatrooms.py'):
     lgcts = eval(read_file(GROUPCHAT_CACHE_FILE))
     gc=lgcts[source[1]]
...
register_command_handler(handler_rejoinall, 'rejoinall', ['admin','muc','all'], 300, 'Rejoin bot to conferences according to database.', 'rejoinall', ['rejoinall'])

register_command_handler(handler_changestatus, 'setstatus', ['muc','admin','all'],300, 'Change bot status in the current conference, if two parameters are not mentioned, bots will use the default status.', 'setstatus[online|chat|away|xa|dnd] [message]', ['setstatus chat','setstatus dnd','setstatus away meeting!!','setstatus'])
