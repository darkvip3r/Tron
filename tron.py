...
def call_command_handlers(command, type, source, parameters, callee):
	...
	if COMMAND_HANDLERS.has_key(command):
		if has_access(source, real_access, source[1]):
			...
			reply(type, source, u'you have no admin rights')
	...
	for possibility in possibilities:
		if possibility[-3:].lower() == '.py' or '.txt':
			try:
				...
				data = fp.read(20)
				if data == '#===istalismanplugin' or data == '#===isfatalplugin===' or data == '#===PyTron===' :
...
"""
def join_groupchat(groupchat=None, nick=DEFAULT_NICK, passw=None):
        confstatus=[u'chat',u'Hi there! I\'m Tron. you can add me and join me to any room you want. no admin rights needed.  Write "%shelp" and follow the instructions. (Made by Marcus)' % (COMM_PREFIX)]#ketik "help" tanpa tanda kutip, untuk mendapatkan bantuan!
...
def iqHnd(con, iq):
	...
	if not iq.getType() == 'error':
		if iq.getTags('query', {}, xmpp.NS_VERSION):
			...
			query.setTagData('version', BOT_VER['botver']['ver'] % (BOT_VER['rev']+last_rev))
			...
			if iqtype == 'set':
				...
				if subs and fromjid:
					...
					if subs == 'none':
						...
						if MSUBS_QUERY:
							msg(ADMINS[0],u'Contact %s refused to authorize and was not added to the roster!' % (fromjid))
							MSUBS_QUERY = 0
